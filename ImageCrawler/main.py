...
class AutoCrawler:
    ...
    def download_images(self, keyword, links, site_name, max_count=0):
        ...
        with open(os.path.join(self.download_path.replace('"', ''),
                               site_name+'_'+keyword+'.csv'), 'w', newline="") as csv_f:
            writer = csv.writer(csv_f)

            for index, link_tags in enumerate(links):
                ...
                try:
                    ...
                    #                                    str(index).zfill(4))
                    no_ext_path = '{}/{}/{}'.format(self.download_path.replace('"', ''), keyword, str(index).zfill(4))
                    ...
                    if ext2 is None:
                        ...
                        # TODO: csv write path tags
                        if ext != ext2:
                            path2 = no_ext_path + '.' + ext2
                            os.rename(path, path2)
                            print('Renamed extension {} -> {}'.format(ext, ext2))
                            row = [keyword, str(index).zfill(4) + '.' +ext2] + tags
                        else:
                            row = [keyword, str(index).zfill(4) + '.' +ext] + tags
                        writer.writerow(row)
